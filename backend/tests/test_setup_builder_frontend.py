...
def test_formal_frontend_supports_direct_case_setup_and_solve():
    ...
    assert 'id="platformSchoolNewButton" type="button" onclick="ScheduleAuth.newSchool()" hidden>取消編輯</button>' in html
    ...
    assert "newSchoolButton.hidden = !editingSchool" in auth
    assert 'newSchoolButton.textContent = "取消編輯"' in auth
