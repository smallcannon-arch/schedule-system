...
### 第三方審查 M1：Warning 的 school context

Claude 指出原測試以 `SimpleNamespace(id="school-a")` 注入 `_school`，應回頭確認真實類別。查證結果：

- `FirestoreScheduleStore.__init__()` 明確執行 `self._school = client.collection("schedule_schools").document(school_id)`。
- 使用 Firestore 2.28.0 真實 SDK、`AnonymousCredentials` 且不連網建構後，物件型態為 `google.cloud.firestore_v1.document.DocumentReference`。
- `store._school.id == "school-a"`。
- `store._school.path == "schedule_schools/school-a"`。
- 上一輪 Native Firestore 實測 warning 亦已輸出實際 `codex-contract-...` school ID，而非 `unknown`。

因此正式 warning 引用正確，無需修改執行程式。另新增真實 SDK 建構子契約測試，避免未來屬性名稱或型態改動後只靠 fake 注入而漏測。

新增測試位於 `backend/tests/test_usage_tracking.py`：
...
此測試直接呼叫真實 `FirestoreScheduleStore.__init__()` 與 Firestore 2.28.0 SDK 類別；`AnonymousCredentials` 只避免連網驗證，不替換 `DocumentReference`，也未手動注入 `_school`。
...
- 原始 hardened commit: `936c039b9561321e05c7674aa9240e01b4334563`
- 第二輪 Firestore 修正 commit: `c38400683e939799c806a43d75411c9605c412f8`
- **本文件目前的程式審查基準：`292315ade46ed79c27b6a61d9c1b52cee8e7605c`**
- `292315a` commit message: `Close Firestore school context review`

PR #1 自 `main` 起的完整 commit 順序：

```text
936c039 Harden platform usage overview
49b476b Document usage overview review fixes
c384006 Verify Firestore usage contracts
f25ea6a Document Firestore contract verification
292315a Close Firestore school context review
```

其中 `49b476b`、`f25ea6a` 為審查文件 commit；`292315a` 新增 M1 真實 SDK 契約測試並補正文檔，未修改正式程式邏輯。

### 第二輪 Firestore 程式 Diff 統計（`c384006`）
...
### M1 結案 Commit Diff 統計（`292315a`）

```text
 AI_REVIEW_USAGE_OVERVIEW_HARDENED_ROUND2.md | 27 +++++++++++++++++++--------
 backend/tests/test_usage_tracking.py        | 13 +++++++++++++
...
- M1 commit: <https://github.com/smallcannon-arch/schedule-system/commit/292315ade46ed79c27b6a61d9c1b52cee8e7605c>
- M1 raw diff: <https://github.com/smallcannon-arch/schedule-system/commit/292315ade46ed79c27b6a61d9c1b52cee8e7605c.diff>
...
git show --no-ext-diff --unified=3 292315ade46ed79c27b6a61d9c1b52cee8e7605c
```
...
```text
pytest.exe -o pythonpath=. -q \
  tests/test_usage_tracking.py tests/test_schedule_auth_frontend.py

47 passed, 2 skipped in 1.35s
```

### 最新分支本機全套測試

```text
pytest.exe -o pythonpath=. -q tests

177 passed, 2 skipped in 4.12s
```
...
- Python: `3.12`
- Head commit: `292315ade46ed79c27b6a61d9c1b52cee8e7605c`
- Test job: `pass`，46 秒
- Deploy job: `skipping`
- Run: <https://github.com/smallcannon-arch/schedule-system/actions/runs/29378960742>
...
> 凡被引用為審查證據的測試結果、實測輸出或程式變更，其所在 commit 必須列入審查基準，並提供可取得的 diff、檔案內容或其他可驗證證據。

本文件記載的 `47 passed, 2 skipped` 與 `177 passed, 2 skipped` 均是在 `292315a` 所含 M1 測試存在時執行；該測試的原文、檔案位置、commit 與 raw diff 已一併列出。

## 合併條件結論

B01 的真實 SDK 驗證、12 筆 aggregation、legacy query、index、乾淨環境與 Python 3.12 CI 均已通過；第三方審查 M1 亦已用真實 `DocumentReference` 契約確認。第二輪技術門檻已滿足，可進入人工審查與合併決策；目前 PR 維持 Draft，尚未合併或部署。
